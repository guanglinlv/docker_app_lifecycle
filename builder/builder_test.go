...
var _ = Describe("Building", func() {
	var (
		...
		dockerRegistryAuthToken    string
		dockerRegistryEmail        string
	...
	JustBeforeEach(func() {
		...
		if len(dockerRegistryAuthToken) > 0 {
			args = append(args, "-dockerRegistryAuthToken", dockerRegistryAuthToken)
		}
		if len(dockerRegistryEmail) > 0 {
			args = append(args, "-dockerRegistryEmail", dockerRegistryEmail)
		}
		...
		Context("with invalid docker registry credentials", func() {
			...
			Context("without auth token", func() {
				BeforeEach(func() {
					...
					dockerRegistryEmail = "mail@example.com"
				})

				It("errors", func() {
					session := setupBuilder()
					Eventually(session.Err).Should(gbytes.Say("missing flags: dockerLoginServer, dockerRegistryAuthToken and dockerRegistryEmail required simultaneously"))
					Eventually(session).Should(gexec.Exit(1))
				})
			})
			...
			Context("with invalid email", func() {
				BeforeEach(func() {
					dockerRegistryAuthToken = "token"
					dockerRegistryEmail = "invalid email"
					dockerLoginServer = "http://loginserver.com"
				})

				It("errors", func() {
					session := setupBuilder()
					Eventually(session.Err).Should(gbytes.Say(regexp.QuoteMeta(fmt.Sprintf("invalid dockerRegistryEmail [%s]", dockerRegistryEmail))))
					Eventually(session).Should(gexec.Exit(1))
				})
			})
