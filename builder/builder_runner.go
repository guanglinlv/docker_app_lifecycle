...
func (builder *Builder) WriteDockerConfig() error {
	if len(builder.DockerLoginServer) > 0 && len(builder.DockerRegistryAuthToken) > 0 && len(builder.DockerRegistryEmail) > 0 {
		fmt.Printf("Saving configuration ...")

		cfgTemplate := `{"%s": {"auth": "%s", "email": "%s"} }`
		config := []byte(fmt.Sprintf(cfgTemplate, builder.DockerLoginServer, builder.DockerRegistryAuthToken, builder.DockerRegistryEmail))
